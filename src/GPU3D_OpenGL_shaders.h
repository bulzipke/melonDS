...
#ifdef OGLRENDERER_GLES
const char* kClearVS = R"(#version 320 es
...
const char* kClearFS = R"(#version 320 es
precision highp float;
precision highp int;
...
const char* kFinalPassVS = R"(#version 320 es
precision highp float;
precision highp int;
...
const char* kFinalPassEdgeFS = R"(#version 320 es
...
{
    mediump vec2 uScreenSize;
    highp int uDispCnt;
    mediump vec4 uToonColors[32];
    mediump vec4 uEdgeColors[8];
    mediump vec4 uFogColor;
    mediump float uFogDensity[34];
    highp int uFogOffset;
    highp int uFogShift;
};

layout(location = 0) out mediump vec4 oColor;

bool isless(highp float a, highp float b)
{
    return a < b;
}

bool isgood(mediump vec4 attr, highp float depth, int refPolyID, highp float refDepth)
{
    int polyid = int(attr.r * 63.0 + 0.5);
    return (polyid != refPolyID && isless(refDepth, depth));
}

void main()
{
    highp ivec2 coord = ivec2(gl_FragCoord.xy);
    int scale = 1;

    highp float depth = texelFetch(DepthBuffer, coord, 0).r;
    ...
    if (attr.g != 0.0)
    {
        highp float depthU = texelFetch(DepthBuffer, coord + ivec2(0, -scale), 0).r;
        mediump vec4 attrU = texelFetch(AttrBuffer, coord + ivec2(0, -scale), 0);

        if (isgood(attrU, depthU, polyid, depth))
        {
            ret.rgb = uEdgeColors[polyid >> 3].bgr;
            ret.a = ((uDispCnt & (1<<4)) != 0) ? 0.5 : 1.0;
        }
    }
...
const char* kFinalPassFogFS = R"(#version 320 es
precision mediump float;
precision highp int;
...
mediump vec4 CalculateFog(highp float depth)
{
    highp int idepth = int(depth * 16777216.0);
    int densityid;
    highp uint udepth;
    ...
    {
        udepth = uint(idepth - uFogOffset);
        udepth = (udepth >> 2u) << uint(uFogShift);
    ...
    mediump float density = mix(uFogDensity[densityid], uFogDensity[densityid+1], float(udepth) * 7.62939453e-6);
...
void main()
{
    ...
    mediump vec4 ret = vec4(0.0);
    if (attr.b != 0.0) ret = CalculateFog(depth);
...
layout(location = 0) in highp uvec4 vPosition;
layout(location = 1) in uvec4 vColor;
layout(location = 2) in highp ivec2 vTexcoord;
layout(location = 3) in ivec3 vPolygonAttr;

smooth out mediump vec4 fColor;
smooth out highp vec2 fTexcoord;
flat out highp ivec3 fPolygonAttr;
)";
...
flat in highp ivec3 fPolygonAttr;
...
mediump vec4 TextureFetch_A3I5(highp ivec2 addr, highp ivec4 st, int wrapmode)
{
    st.x = TexcoordWrap(st.x, st.z, wrapmode>>0);
    st.y = TexcoordWrap(st.y, st.w, wrapmode>>1);

    addr.x += ((st.y * st.z) + st.x);
    highp ivec4 pixel = ivec4(texelFetch(TexMem, ivec2(addr.x&0x3FF, addr.x>>10), 0));
    ...
    return vec4(color.rgb, float(pixel.a) * 0.032258);
}

mediump vec4 TextureFetch_I2(highp ivec2 addr, highp ivec4 st, int wrapmode, mediump float alpha0)
{
    st.x = TexcoordWrap(st.x, st.z, wrapmode>>0);
    st.y = TexcoordWrap(st.y, st.w, wrapmode>>1);
...
mediump vec4 TextureFetch_I4(highp ivec2 addr, highp ivec4 st, int wrapmode, mediump float alpha0)
{
    st.x = TexcoordWrap(st.x, st.z, wrapmode>>0);
    st.y = TexcoordWrap(st.y, st.w, wrapmode>>1);
...
mediump vec4 TextureFetch_I8(highp ivec2 addr, highp ivec4 st, int wrapmode, mediump float alpha0)
{
    st.x = TexcoordWrap(st.x, st.z, wrapmode>>0);
    st.y = TexcoordWrap(st.y, st.w, wrapmode>>1);
...
mediump vec4 TextureFetch_Compressed(highp ivec2 addr, highp ivec4 st, int wrapmode)
{
    st.x = TexcoordWrap(st.x, st.z, wrapmode>>0);
    st.y = TexcoordWrap(st.y, st.w, wrapmode>>1);

    addr.x += ((st.y & 0x3FC) * (st.z>>2)) + (st.x & 0x3FC) + (st.y & 0x3);
    highp ivec4 p = ivec4(texelFetch(TexMem, ivec2(addr.x&0x3FF, addr.x>>10), 0));
    ...
    if (val == 0)
    {
        return vec4(texelFetch(TexPalMem, ivec2(addr.y&0x3FF, addr.y>>10), 0).rgb, 1.0);
    }
    else if (val == 1)
    {
        addr.y++;
    ...
    else if (val == 2)
    {
        if (palinfo == 1)
        {
            mediump vec3 c0 = texelFetch(TexPalMem, ivec2(addr.y&0x3FF, addr.y>>10), 0).rgb;
            addr.y++;
            mediump vec3 c1 = texelFetch(TexPalMem, ivec2(addr.y&0x3FF, addr.y>>10), 0).rgb;
            return vec4((c0 + c1) * 0.5, 1.0);
        }
        else if (palinfo == 3)
        {
            mediump vec3 c0 = texelFetch(TexPalMem, ivec2(addr.y&0x3FF, addr.y>>10), 0).rgb;
            addr.y++;
            mediump vec3 c1 = texelFetch(TexPalMem, ivec2(addr.y&0x3FF, addr.y>>10), 0).rgb;
            return vec4((c0*5.0 + c1*3.0) * 0.125, 1.0);
        }
        else
        {
            addr.y += 2;
    ...
    else
    {
        ...
        else if (palinfo == 3)
        {
            ...
            addr.y++;
            mediump vec3 c1 = texelFetch(TexPalMem, ivec2(addr.y&0x3FF, addr.y>>10), 0).rgb;
            return vec4((c0*3.0 + c1*5.0) * 0.125, 1.0);
        }
        else
        {
            return vec4(0.0);
...
mediump vec4 TextureFetch_A5I3(highp ivec2 addr, highp ivec4 st, int wrapmode)
{
    st.x = TexcoordWrap(st.x, st.z, wrapmode>>0);
    st.y = TexcoordWrap(st.y, st.w, wrapmode>>1);
...
mediump vec4 TextureFetch_Direct(highp ivec2 addr, highp ivec4 st, int wrapmode)
{
    st.x = TexcoordWrap(st.x, st.z, wrapmode>>0);
    st.y = TexcoordWrap(st.y, st.w, wrapmode>>1);

    addr.x += ((st.y * st.z) + st.x) << 1;
    highp int rL = int(texelFetch(TexMem, ivec2(addr.x&0x3FF, addr.x>>10), 0).r);
    addr.x++;
    highp int rH = int(texelFetch(TexMem, ivec2(addr.x&0x3FF, addr.x>>10), 0).r);

    mediump vec4 color;
    color.r = float(rL & 0x1F) * 0.032258;
    color.g = float((rL >> 5) | ((rH & 0x03) << 3)) * 0.032258;
    color.b = float((rH & 0x7C) >> 2) * 0.032258;
    color.a = float(rH >> 7);

    return color;
}

mediump vec4 TextureLookup_Nearest(highp vec2 st)
{
    highp int attr = int(fPolygonAttr.y);
    highp int paladdr = int(fPolygonAttr.z);

    mediump float alpha0 = ((attr & (1<<29)) != 0) ? 0.0 : 1.0;

    highp int tw = 8 << ((attr >> 20) & 0x7);
    highp int th = 8 << ((attr >> 23) & 0x7);
    highp ivec4 st_full = ivec4(ivec2(st), tw, th);

    highp ivec2 vramaddr = ivec2((attr & 0xFFFF) << 3, paladdr);
    int wrapmode = (attr >> 16);

    int type = (attr >> 26) & 0x7;
    switch(type) {
        case 5: return TextureFetch_Compressed(vramaddr, st_full, wrapmode);
        case 2: return TextureFetch_I2(vramaddr, st_full, wrapmode, alpha0);
        case 3: return TextureFetch_I4(vramaddr, st_full, wrapmode, alpha0);
        case 4: return TextureFetch_I8(vramaddr, st_full, wrapmode, alpha0);
        case 1: return TextureFetch_A3I5(vramaddr, st_full, wrapmode);
        case 6: return TextureFetch_A5I3(vramaddr, st_full, wrapmode);
        default: return TextureFetch_Direct(vramaddr, st_full, wrapmode);
    }
}
...
mediump vec4 FinalColor()
{
    ...
    if (blendmode == 2)
    {
        if ((uDispCnt & (1<<1)) == 0)
        {
            vcol.rgb = uToonColors[int(vcol.r * 31.0)].rgb;
        }
        else
        {
            vcol.rgb = vcol.rrr;
...
#else 
const char* kClearVS = R"(#version 140
...
const char* kClearFS = R"(#version 140
...
const char* kFinalPassEdgeFS = R"(#version 140
...
const char* kFinalPassFogFS = R"(#version 140
