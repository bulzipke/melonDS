...
void main()
{
    ivec4 pixel = ivec4(texelFetch(ScreenTex, ivec2(fTexcoord), 0) * 255.5);
    float _3dxpos = float(u3DXPos);

    ivec4 mbright = ivec4(texelFetch(ScreenTex, ivec2(256*3, int(fTexcoord.y)), 0) * 255.5);
    int dispmode = mbright.b & 0x3;

    if (dispmode == 1)
    {
        ivec4 val1 = pixel;
        ivec4 val2 = ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(256,0), 0) * 255.5);
        ivec4 val3 = ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(512,0), 0) * 255.5);
        ...
        vec4 _3draw = texelFetch(_3DTex, ivec2(vec2(xpos, ypos) * float(u3DScale)), 0);
        ivec4 _3dpix = ivec4(_3draw.rgba * vec4(63.5, 63.5, 63.5, 31.5));

        if (compmode == 4) // 3D on top, blending
        {
            if (_3dpix.a > 0) {
                eva = (_3dpix.a & 0x1F) + 1;
                evb = 32 - eva;
        ...
        else if (compmode == 1) // 3D on bottom, blending
        {
            if (_3dpix.a > 0) {
                eva = val3.g; evb = val3.b;
                val1 = ((val1 * eva) + (_3dpix * evb)) >> 4;
                val1 = min(val1, 0x3F);
            } else val1 = val2;
        }
        else if (compmode <= 3) // 3D on top, normal/fade
        {
            if (_3dpix.a > 0) {
                evy = val3.g;
                val1 = _3dpix;
    ...
    if (dispmode != 0)
    {
        ...
        if (brightmode == 1) // Up (밝게)
            pixel += ((ivec4(0x3F,0x3F,0x3F,0) - pixel) * evy) >> 4;
        else if (brightmode == 2) // Down (어둡게)
            pixel -= (pixel * evy) >> 4;
    }
...
const char* kCompositorFS_Linear = R"(#version 320 es
precision highp float;
precision highp int;
precision highp sampler2D;
...
{
    return ivec4(texelFetch(_3DTex, ivec2(pos*u3DScale), 0).rgba
         * vec4(63,63,63,31));
...
void main()
{
    ...
    if (dispmode == 1)
    {
        ...
        ivec4 p01 = GetFullPixel(ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(xdisp+0  ,0), 0) * 255.5),
                                 ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(xdisp+256,0), 0) * 255.5),
                                 ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(xdisp+512,0), 0) * 255.5),
                                 _3dpix);

        ivec4 p10 = GetFullPixel(ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(0+0  ,ydisp), 0) * 255.5),
                                 ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(0+256,ydisp), 0) * 255.5),
                                 ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(0+512,ydisp), 0) * 255.5),
                                 _3dpix);

        ivec4 p11 = GetFullPixel(ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(xdisp+0  ,ydisp), 0) * 255.5),
                                 ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(xdisp+256,ydisp), 0) * 255.5),
                                 ivec4(texelFetch(ScreenTex, ivec2(fTexcoord) + ivec2(xdisp+512,ydisp), 0) * 255.5),
                                 _3dpix);
...
const char* kCompositorVS_xBRZ = R"(#version 320 es
precision highp float;
precision highp int;
...
const char* kCompositorFS_xBRZ = R"(#version 320 es
precision highp float;
precision highp int;
precision highp sampler2D;
...
{
    ivec4 pixel = ivec4(texelFetch(ScreenTex, ivec2(texcoord) + ivec2(level*256,0), 0) * 255.5);
